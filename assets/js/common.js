...
const mobileDeviceDetect = function mobileDeviceDetect() {
...
const autoModal = () => {
    ...
    if ($.cookie('autoModal_shown') == null) {
        $.cookie('autoModal_shown', 'yes', { expires: 3, });
        runModal();
    }
    // runModal();
    function runModal(){
...
const toTopButton = () => {
    ...
    $(window).scroll(function() {
        if ($(window).scrollTop() > 400) {
            scrollToTopButton.addClass("to-top_show");
            scrollToTopButton.removeClass("to-top_hide");
        } else {
            scrollToTopButton.removeClass("to-top_show");
...
const formValidation = () => {
    $(".send-form_validate").each(function(){
        $(this).validate({
...
const cardFavorits = () => {
    ...
    $('.js-card-fav').on('click', function(){
        let _this = $(this);

        if( _this.hasClass('_active')){
            _this.removeClass('_active');
            _this.find('._path-one').css('fill', '');
        } else {
            _this.addClass('_active');
            _this.find('._path-one').css('fill', '#D1202A');
        }
    });
